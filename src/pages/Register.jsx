...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.full_name || !formData.email || !formData.password || !formData.phone) {
      setError('Semua kolom wajib diisi, termasuk nomor HP');
      return;
    }
    ...
    try {
      ...
      const { data } = response.data;
      const redirectTo = data.redirect_to || (data.next_step === 'dashboard' ? '/dashboard' : '/onboarding');
        ...
        <div className="bg-white dark:bg-[#1f2028] rounded-[24px] shadow-sm border border-gray-100 dark:border-[#2e303a] w-full max-w-[900px] flex flex-col md:flex-row overflow-hidden">
          ...
          <div className="hidden md:flex md:w-[45%] bg-[#05A845] p-10 flex-col justify-center relative overflow-hidden">
            ...
            <div className="z-10 mt-[-20px]">
              ...
              <p className="text-[#EAF6ED] text-[15px] leading-relaxed pr-4">
                Take control of your finances and start building a better future with our community.
              </p>
            </div>
          </div>
          ...
          <div className="w-full md:w-[55%] p-8 sm:p-12 flex flex-col justify-center bg-white dark:bg-[#1f2028]">
            ...
            <div className="text-center mb-8">
              <h2 className="text-[22px] font-bold text-[#1A1A1A] dark:text-white mb-1">Create New Account</h2>
              <p className="text-[#666666] dark:text-gray-400 text-[14px]">Fill in your details to get started.</p>
            </div>
            ...
            <p className="text-center text-[#666666] dark:text-gray-400 text-[14px] mt-6">
              Already have an account? <Link to="/login" className="text-[#05A845] hover:text-[#048A38] font-bold">Sign In</Link>
            </p>
